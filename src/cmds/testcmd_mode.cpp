...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/13 18:40:46 by lrandria          #+#    #+#             */
/*   Updated: 2022/12/13 21:15:11 by lrandria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		addModes(Client &client, ChannelManager::iterator itChan, std::vector< std::string > args) {
	...
	if (args[1] == "+i" || args[1] == "i") {
		...
		client.reply<ERR_UNKNOWNMODE>(args[1]);
}
...
void	handleModes(Client &client, Server &server, std::vector< std::string > args) {
	...
	int pos = allowedChars.find(modes[0]);
	if (pos != std::string::npos && modes.length() > 1)
		addModes(client, itChan, args);
	else
		client.reply<ERR_UNKNOWNMODE>(modes);
}
...
void	    cmd_mode(CommandContext &context) {
	...
	if (args.empty()) {
		...
		client.reply<ERR_NOSUCHCHANNEL>(args[0]); //why is this working?
}
